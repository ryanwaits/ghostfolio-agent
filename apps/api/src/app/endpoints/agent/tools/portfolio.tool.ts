  ...
  return tool({
    ...
    inputSchema: z.object({
      dateRange: z
        ...
        .optional()
        .describe('Time range for the analysis. Defaults to max (all time).')
    }),
    execute: async ({ dateRange = 'max' }) => {
      try {
        ...
        return { error: `Failed to fetch portfolio: ${error instanceof Error ? error.message : 'unknown error'}` };
      }
    }
  });
}
